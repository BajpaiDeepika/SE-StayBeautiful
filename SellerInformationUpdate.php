	...
	if (isset($_GET['ID']))
	{
		...
		if (isset($_POST['submit']))
		{
			...
			if (is_numeric($_POST['ID']))
			{
				...
				else
				{
					...
					if ($stmt = $conn1->prepare("UPDATE members SET address = ?, city = ?, state = ?, zip = ?, phone = ?
					WHERE sellerid= ?"))
					{
					...
					header("Location: SellerDashboard.php?seller_id=$ID");
				}
			}
		...
		else
		{
			...
			if (is_numeric($_GET['ID']) && $_GET['ID'] > 0)
			{
				...
				if($stmt = $conn1->prepare("SELECT address,city,state,zip,phone FROM members WHERE sellerid= $ID"))
				{
					$stmt->bind_param("sssssi",$address, $city, $state, $zip, $phone, $ID);
					$stmt->execute();
