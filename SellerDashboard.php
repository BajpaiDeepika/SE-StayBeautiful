...
<div class="header_btm">
  ...
  <!-- Start slider -->
			<section>
				<br>
<?php $ID=($_GET['seller_id']); ?>
  <h2 style="color:purple";><strong><center><b>Reports</b></center></strong></h2>
  <form action="RevenueReport.php" method="POST">
   <input type="hidden" name="ID" value=<?php echo $ID; ?>>
  <center><button class="button" type="submit" value="Report">Revenue Report</button></center>
  </form>
  <form action="ItemsSoldReport.php" method="POST">
   <input type="hidden" name="ID" value=<?php echo $ID; ?>>
  <center><button class="button" type="submit" value="Report">Items sold Report</button></center>
  </form>
  <form action="inventory-mgmt.php" method="POST">
  <center><button class="button" type="submit" value="Report">Update Inventory</button></center>
	</form>
  ...
  <?php
	...
	$conn = new mysqli($servername, $username, $password, $dbname);
						...
						$query="Select username FROM members1 where userid={$_GET['seller_id']} limit 1";
	$result = mysqli_query($conn,$query);
	$value = mysqli_fetch_object($result);
	$ID= $_GET['seller_id'];
	if($result = $conn->query("SELECT address,city,state,zip,phone FROM members where sellerid=$ID"))
	{
		if($result->num_rows != 0)
		{
			...
			echo "<tr>";
			echo "<th>Address</th><th>City</th><th>State</th><th>Zip code</th><th>Phone</th>";
			echo "</tr>";
			echo "<tr>";
  ...
  <?php
  echo "<center><a href=SellerInformationUpdate.php?ID=$ID>Update Personal Information</a>";
  ?>
  <!-- footer -->  
  <footer id="aa-footer">
    <!-- footer bottom -->
